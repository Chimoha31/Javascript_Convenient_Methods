...
for (let j = 0; j < products2.length; j++) {
  if (
    products2[j].type == "野菜" &&
    products2[j].quantity > 0 &&
    products2[j].price < 10
  ) {
...
let filteredProducts3 = products2.filter(function (product2) {
  return (
    product2.type == "野菜" && product2.quantity > 0 && product2.price < 10
  );
});
...
let cars2 = [{ name: "プリウス" }, { name: "ノート" }, { name: "アクア" }];
...
function addNumbers2(...numbers2) {
  return numbers2.reduce((previous, number2) => {
  return previous += number2;
  }, 0);
}
